...
@pytest.mark.skipif(not is_uva_available(), reason="UVA is not available.")
@pytest.mark.parametrize("device", CUDA_DEVICES)
@pytest.mark.parametrize("dtype", [torch.int32, torch.float32, torch.bfloat16])
@pytest.mark.parametrize("transposed", [False, True])
def test_require_live_view_succeeds_on_pinned_memory(device, dtype, transposed):
    """When require_live_view=True is passed and the CPU tensor IS pinned,
    the call should succeed and return a live alias."""
    torch.set_default_device(device)
    cpu_tensor = torch.zeros(2, 4, dtype=dtype, device="cpu", pin_memory=True)
    if transposed:
    ...
    assert gpu_view.stride() == cpu_tensor.stride()

    cpu_tensor.fill_(42)
    assert torch.equal(gpu_view.cpu(), cpu_tensor)
    gpu_view.add_(1)
    torch.accelerator.synchronize()
    assert torch.all(cpu_tensor == 43)
